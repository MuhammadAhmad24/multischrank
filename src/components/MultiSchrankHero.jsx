...
import { useLanguage } from "../LanguageContext"; // path apne project ke hisaab se set kar lo

const content = {
    de: {
        badge: "Premium Innenräume • MultiSchrank",
        titleLine1: "Moderne Möbel",
        titleLine2Start: "für",
        titleLine2Highlight: "stilvolle Räume",
        imageAlt: "Luxuriöses Interieur",
    },
    en: {
        badge: "Premium Interiors • MultiSchrank",
        titleLine1: "Modern furniture",
        titleLine2Start: "for",
        titleLine2Highlight: "stylish spaces",
...
export default function MultiSchrankHero() {
    ...
    const imageRadius = useTransform(
        imageProgress,
        [0, 0.3, 0.62, 1],
        isMobile ? [18, 18, 18, 18] : [28, 28, 16, 16]
    );

    const imageScale = useTransform(
        imageProgress,
        [0, 0.3, 0.62, 1],
        isMobile ? [1, 1, 1, 1] : [0.96, 1, 1.03, 1.03]
    );

    const imageOpacity = useTransform(
        imageProgress,
        [0, 0.08, 0.2, 1],
        isMobile ? [1, 1, 1, 1] : [0.55, 0.8, 1, 1]
    );
    ...
    return (
        ...
        >
            ...
            <div className="relative z-20 mb-5 sm:min-h-screen md:mb-0">
                <motion.div
                    initial="hidden"
                    animate="show"
                    variants={container}
                    style={{ y: contentY, opacity: contentOpacity }}
                    className="mx-auto flex w-full max-w-7xl flex-col items-center px-5 pt-30 pb-8 sm:pb-0 text-center sm:px-6 md:min-h-screen md:pt-44 lg:px-10"
                >
                    <motion.div
                        variants={fadeUp}
            ...
            <div
                ref={trackRef}
                className={`relative z-10 ${isMobile ? "h-auto pb-30" : "h-screen md:h-[200vh]"}`}
            >
                <div className={isMobile ? "" : "sticky top-1/2 -translate-y-1/2"}>
                    <motion.div
                        style={{
                            width: imageWidth,
                            scale: imageScale,
